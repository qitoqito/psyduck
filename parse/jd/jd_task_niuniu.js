...
export class Main extends Template {
    ...
    async prepare() {
        this.shareCode({linkId: "LsQNxL7iWDlXUs6cFl-AAg"})
    }
    ...
    async main(p) {
        ...
        if (this.dict[user].unlock && joys.length>0) {
            ...
            for (let i = 0; i<min; i++) {
                ...
                p.log('移动牛牛去工作...', this.haskey(move, 'success'))
            }
        }
        let cashPrize = await this.curl({
                'url': `https://api.m.jd.com/?functionId=gameMyCashPrize&body={"linkId":"${context.linkId}","pageNum":1,"pageSize":10}&t=1741137369937&appid=activities_platform&client=ios&clientVersion=15.0.25`,
                // 'form':``,
                user
            }
        )
        for (let i of this.haskey(cashPrize, 'data.items')) {
            if (i.prizeType == 4 && i.state == 0) {
                let sss = await this.curl({
                        'url': 'https://api.m.jd.com/',
                        form: `functionId=apCashWithDraw&body={"businessSource":"JOY_PARK","base":{"id":${i.id},"business":"joyPark","poolBaseId":${i.poolBaseId},"prizeGroupId":${i.prizeGroupId},"prizeBaseId":${i.prizeBaseId},"prizeType":${i.prizeType},"activityId":"${i.activityId}"},"linkId":"${context.linkId}","inviter":""}&t=1741137369937&appid=activities_platform&client=ios&clientVersion=15.0.25`,
                        user
                    }
                )
                if (this.haskey(sss, 'data.message', "无效的openId，当前pin尚未绑定微信")) {
                    p.log("无效的openId，当前pin尚未绑定微信")
                    break
                }
                p.msg(`提现${i.prizeValue} ${this.haskey(sss, 'data.message')}`)
            ...
            if (this.dict[user].joy[i].length>1) {
                ...
                for (let k = 0; k<spl.length; k++) {
                    ...
                    if (this.haskey(merge, 'data.joyVO.id')) {
                        ...
                        joyss[i] = spl[k + 1] || []
                        p.log(`合成等级${parseInt(i) + 1}的牛牛成功...`, JSON.stringify(joyss))
                    }
                    // p.log(`合成等级${parseInt(i) + 1}的牛牛成功...`, this.dict[user].joy[parseInt(i) + 1])
                }
            }
        }
        if (isChange>0) {
                ...
                if (!this.haskey(joy, `${i}.0`)) {
                    ...
                    if (joyInfo && this.dict[user].coin>0 && joyInfo.consume>0 && joyInfo.consume<this.dict[user].coin) {
                        ...
                        if (this.haskey(buy, 'data')) {
                            // p.log(`购买等级${i}的牛牛成功...`, this.dict[user].joy[i])
                            p.log(`购买等级${i}的牛牛成功...`, JSON.stringify(this.dict[user].joy))
                        }
                        else {
                            p.log(`购买等级${i}的牛牛失败...`)
                            break
                ...
                for (let i in this.dict[user].joy) {
                    if (this.dict[user].joy[i] && this.dict[user].joy[i].length>1 && this.dict[user].joy[i].length % 2 == 0) {
                        ...
                        for (let k of spl) {
                            ...
                            await this.joyList(p)
                            // p.log(`合成等级${parseInt(i) + 1}的牛牛成功...`, this.dict[user].joy[parseInt(i) + 1])
                            p.log(`合成等级${parseInt(i) + 1}的牛牛成功...`, JSON.stringify(this.dict[user].joy))
                        }
                    }
                }
            }
