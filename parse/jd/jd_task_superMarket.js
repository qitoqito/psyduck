...
export class Main extends Template {
    ...
    async main(p) {
        ...
        let info = await this.curl({
                ...
                'form': `appid=jd-super-market&t=1748855058331&functionId=atop_channel_interactive_info&client=m&body={"bizCode":"cn_retail_jdsupermarket","scenario":"sign","babelChannel":"ttt32","isJdApp":"1","isWx":"0"}`,
                user,
                algo: {
                    appId: '33c74'
                }
                ...
                if (i.assignmentName.includes('邀请')) {
                    ...
                    if (users) {
                        ...
                        let help = await this.curl({
                                'form': `appid=jd-super-market&t=1742122986378&functionId=atop_channel_complete_task&client=m&body={"bizCode":"cn_retail_jdsupermarket","scenario":"sign","assignmentType":"${i.assignmentType}","encryptAssignmentId":"${i.encryptAssignmentId}","itemId":"${itemId}","assistFlag":true,"babelChannel":"ttt1","isJdApp":"0","isWx":"1"}`,
                                user,
                                algo: {
                                    appId: '51113'
                                }
        ...
        let calendar = await this.curl({
                ...
                'form': `appid=jd-super-market&t=1748931681904&functionId=atop_channel_sign_calendar&client=m&body={"bizCode":"cn_retail_jdsupermarket","scenario":"sign","babelChannel":"ttt32","isJdApp":"1","isWx":"0"}&clientVersion=15.1.50`,
                user
            }
        )
            ...
            for (let i of calendar.data.floorData.items) {
                if (i.signToken) {
                    let sign = await this.curl({
                            ...
                            'form': `appid=jd-super-market&t=1713230766545&functionId=atop_channel_sign_in&client=m&body={"signToken":"${i.signToken}","channelFollowStatus":1,"bizCode":"cn_retail_jdsupermarket","scenario":"sign","babelChannel":"ttt1","isJdApp":"1","isWx":"0"}`,
                            user,
                            algo: {
                                appId: 'b8fc7',
                                expire: {
